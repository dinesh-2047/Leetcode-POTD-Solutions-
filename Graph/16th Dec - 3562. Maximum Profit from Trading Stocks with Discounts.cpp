...
public:
        ...
        for (int parentBought = 0; parentBought <= 1; ++parentBought) {
            ...
            if (price <= budget) {
                ...
                for (const auto& child :
                     childrenStatesProfit) { // from all child
                    vector<int> temp(budget + 1, 0);
    ...
    int maxProfit(int n, vector<int>& present, vector<int>& future,
        ...
        // statesProfit[u][0][b] = profit at u when it's parent had not bought
        // stock statesProfit[u][1][b] = profit at v when it's parent bought had
        // bought stock
